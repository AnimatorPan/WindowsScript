...
class VideoLoadWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            if is_gif:
                # GIF文件：获取实际帧数并提取所有帧
                actual_frames = self._get_gif_frame_count()
                print(f"[性能] GIF文件实际帧数: {actual_frames}")
                
                if actual_frames > 0:
                    # 提取所有帧
                    num_frames = actual_frames
                    # 生成时间点，确保覆盖所有帧
                    keyframe_times = [i * duration / num_frames for i in range(num_frames)]
                else:
                    # 无法获取帧数时，使用默认策略
                    num_frames = min(int(duration * 10), 100)
                ...
                # - 21秒以上：提取1/3的帧
                if duration <= 3:
                    num_frames = int(duration * 30)  # 每秒30帧
                elif duration <= 10:
                    num_frames = int(duration * 30 * 2 / 3)  # 2/3的帧
                elif duration <= 20:
                    num_frames = int(duration * 30 / 2)  # 1/2的帧
                else:
                    num_frames = int(duration * 30 / 3)  # 1/3的帧
                
                # 限制最大帧数为300，避免内存溢出
                num_frames = min(num_frames, 300)
                # 至少提取10帧
                num_frames = max(num_frames, 10)
                
                # 生成关键帧时间点
                keyframe_times = [i * duration / num_frames for i in range(num_frames + 1)]
            
            print(f"[性能] 视频时长 {duration:.1f}s, 计划提取 {len(keyframe_times)} 帧")
            
            # 5. 并行提取所有关键帧
            t0 = time.time()
            keyframe_frames = self._extract_keyframes_parallel(keyframe_times)
            print(f"[性能] 实际提取 {len(keyframe_frames)} 关键帧: {(time.time()-t0)*1000:.0f}ms")
    ...
    def _get_gif_frame_count(self) -> int:
        """获取GIF文件的实际帧数"""
        try:
            # 使用ffprobe获取帧数
            cmd = [
            ...
            # 查找frame= xxx fps 模式
            import re
            frames_match = re.findall(r'frame=\s*(\d+)', stderr)
            if frames_match:
                return int(frames_match[-1])
            
            return 0
            
        except Exception as e:
            print(f"[警告] 获取GIF帧数失败: {e}")
            return 0
    ...
    def _extract_keyframes_parallel(self, time_positions: list) -> dict:
        ...
        # 1. 先提取第一帧（时间0）用于立即显示
        first_time = time_positions[0]
        first_frame_path = os.path.join(self.temp_dir, "kf_0000.jpg")
        ...
        try:
            ...
            if os.path.exists(first_frame_path) and os.path.getsize(first_frame_path) > 0:
                frames[first_time] = first_frame_path
                # 发送首帧就绪信号
                self.first_frame_ready.emit(first_frame_path)
                
        except Exception as e:
            print(f"[警告] 提取首帧失败: {e}")
        
        # 2. 并行提取其余帧
        remaining_positions = time_positions[1:]
        total = len(remaining_positions)
        completed = 0
        ...
        def extract_single(args):
            ...
            frame_path = os.path.join(self.temp_dir, f"kf_{idx+1:04d}.jpg")
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = {executor.submit(extract_single, (i, t)): i for i, t in enumerate(remaining_positions)}
...
class VideoPlayer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.sorted_frame_times = []  # 排序后的帧时间列表
    ...
    def _on_all_keyframes_ready(self, frames: dict, info: dict):
        ...
        # 初始化排序后的帧时间列表
        self.sorted_frame_times = sorted(self.keyframe_frames.keys())
    ...
    def _update_playback(self):
        ...
        # 使用帧索引播放，确保平滑循环
        if not self.sorted_frame_times:
            return
        ...
        # 循环：当到达最后一帧时回到第一帧
        if self.current_frame_index >= len(self.sorted_frame_times):
            self.current_frame_index = 0
        
        # 获取当前帧时间并显示
        frame_time = self.sorted_frame_times[self.current_frame_index]
        self.position = frame_time
        
        # 更新UI
        self.progress_slider.setValue(int(self.position / self.duration * 1000))
        self._update_time_label()
        self._show_frame(self.keyframe_frames[frame_time])
    ...
    def _on_slider_moved(self, value):
        ...
        # 更新当前帧索引到最接近的位置
        if self.sorted_frame_times:
            closest_idx = 0
            min_diff = float('inf')
            for i, t in enumerate(self.sorted_frame_times):
                diff = abs(t - self.position)
                if diff < min_diff:
                    min_diff = diff
                    closest_idx = i
            self.current_frame_index = closest_idx
            self._show_frame(self.keyframe_frames[self.sorted_frame_times[closest_idx]])
